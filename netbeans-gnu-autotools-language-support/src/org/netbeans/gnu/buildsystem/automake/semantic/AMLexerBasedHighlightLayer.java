...
import org.netbeans.gnu.buildsystem.automake.semantic.AMColoringAttributes.Coloring;
...
class AMLexerBasedHighlightLayer extends AbstractHighlightsContainer {
    ...
    private AMLexerBasedHighlightLayer(Document doc) {
        this.doc = new WeakReference<>(doc);
        this.colorings = Collections.emptyMap();
        this.bag = new OffsetsBag(doc);
    }

    public static AMLexerBasedHighlightLayer getLayer(Class<?> id, Document doc) {
        AMLexerBasedHighlightLayer layer = (AMLexerBasedHighlightLayer) doc.getProperty(id);

        if (layer == null) {
            doc.putProperty(id, layer = new AMLexerBasedHighlightLayer(doc));
        }
    ...
    public void setColorings(
        ...
        for (Token<? extends TokenId> token : addedTokens) {
            ...
            if (logger.isLoggable(Level.FINE)) {
                ...
                for (AMColoringAttributes attrib : coloring) {
    ...
    synchronized AttributeSet getColoring(Coloring c) {
        ...
        if (a == null) {
            CACHE.put(c, a = AMColoringManager.getColoringImpl(c));
        }
